...
inline static int
s_send_i(void *socket, const char *string, int flags = 0) {
	int rc;
	zmq_msg_t message;
...
inline static bool
s_send_b(zmq::socket_t & socket, const std::string & string, int flags = 0) {
...
inline static int
s_sendmore_i(void *socket, char *string) {
	int rc;
	zmq_msg_t message;
	zmq_msg_init_size(&message, strlen(string));
	memcpy(zmq_msg_data(&message), string, strlen(string));
...
//  Sends string as 0MQ string, as multipart non-terminal
inline static bool
s_sendmore_b(zmq::socket_t & socket, const std::string & string) {
