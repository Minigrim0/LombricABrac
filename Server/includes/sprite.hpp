...
public:
	...
	void setMovement(double speedX, double speedY, double gravity, int type, int t);//type = type de mouvement (default=PARABOLLE)
	bool isInMovement();
	void deleteMovement();

	//update return false si le sprite est 'mort' (plus de vie/doit exploser,...)
	virtual bool update(Map* carte, int t) = 0;//update la position en fonction du mouvement de l'objet et met dans pos les nouvelles coordonnées
	virtual void deathMove(Map* inf, Partie* p, std::vector<Sprite*> s, int t)=0;//ce que les sprites font quand ils meurent (en vrai ce sera surement que des explosion)
...
public:
	...
	bool update(Map* carte, int t) override;//update laa position
	...
	void deathMove(Map* inf, Partie* p,std::vector<Sprite*> s, int t) override;
...
public:
	...
	bool update(Map* carte, int t) override;
	void deathMove(Map* inf, Partie* p, std::vector<Sprite*> s, int t) override;
