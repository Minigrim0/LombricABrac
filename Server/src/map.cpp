...
std::vector<int> Map::explose(std::vector<Sprite*> lombVect, int xExplosion, int yExplosion, int radius, int degat, int vitesse, int t){
  std::vector<int> res;
	...
	for(auto sp=lombVect.begin();sp!=lombVect.end();++sp){//parcours tous les sprites
		if((*sp)->getId() != 0){//alors c'est un lombric
			...
			if(dist <= radius){//seuls les lombrics dans l'exlosion sont touchés
				...
				lomb->setMovement(speedX, speedY, GRAVITY, PARABOLLE, t);
			}
