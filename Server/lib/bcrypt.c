...
int main(void)
{
	...
	const char pass[] = "we are number one";
	const char hash1[] = "$2a$12$qQGq70jzDAZ3QzkmC8qtFe8W.E0dKzyT1KyoErh1xhHD7UOlE5EDC";
	const char hash2[] = "$2a$12$lfJUFXb2gwuhlwhlEti4iO15kMN/4Yv4fuMXrSqcwUg3N5cIL1FFK";
	...
	before = clock();
	ret = (bcrypt_checkpw(pass, hash3) == 0);
	after = clock();
	printf("Second hash check with bcrypt_checkpw: %s\n", ret?"OK":"FAIL");
	printf("Time taken: %f seconds\n",
	       (double)(after - before) / CLOCKS_PER_SEC);
